...
Q1_diff = func_Q_with_dict(
    F_diff, v1, "v1_diff", "Q1"
)  # (при дифференциальной схеме)требуемый расход при выдвижении штока
...
def selection_D_and_d():
    ...
    _reference = (
        "задаём диаметр штока d2 исходя из усилия выдвижения "
    ...
    if var_p == direction[0]:
        ...
        if metadata.get("P2") < metadata.get("P1", 0) and d < metadata["d1"]:
            ...
            msg_for_d2 = (
                ...
                "равный {}\n\n".format(name_par_cyl.get("P1"), name_par_cyl.get("P2"), d1)
                + msg_for_d2
            )
        else:
            metadata["d1"] = d
