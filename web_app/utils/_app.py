...
def get_all_parameters(main_window=None, file_id=None, metadata=None, file_name=None):
    def _get_all_parameters():
        ...
        btn_all_parameters = tk.Button(
            main_window,
            text="Отобразить параметры",
            font=(font[0], 12),
            command=get_all_param,
            **btn_master,
        )
        ...
        def change_param():
            try:
                ...
                txt_param.insert(
                    0.0,
                    "ошибка синтаксиса файла json(запятые, двоеточия)\n" "побробуйте еще",
                )

        btn_change_parameters = tk.Button(
            main_window,
            text="Редактировать & сохранить",
            font=(font[0], 12),
            command=change_param,
            **btn_master,
        )
